...
from helpers import get_sim_args, run_on, create_if_needed
...
def packet_checker(packet, phy):
    if packet.dst_mac_addr == high_priority_mac_addr:
        phy.timeout_monitor.packet_received()


class TimeoutMonitor(xmostest.SimThread):

    def __init__(self, initial_delay, packet_period, expect_count):
...
def do_test(mac, rx_clk, rx_phy, tx_clk, tx_phy):
    ...
    ifg_bytes = 12
    preamble_bytes = 8
    packet_bytes = data_bytes + ifg_bytes + preamble_bytes
    packets_per_second = slope / (data_bytes * 8)
    ...
    timeout_monitor = TimeoutMonitor(20000, packet_period, num_expected_packets)
