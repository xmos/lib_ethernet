             ...
             client control_if ctrl)
{
  ...
  while (!done) {
    ...
    case sin_char_array(c_rx_hp, (char *)&packet_info, sizeof(packet_info)):
      ...
      c_tx_hp <: packet_info.len;
      sout_char_array(c_tx_hp, rxbuf, packet_info.len);
      break;
...
int main()
{
  ...
  par {
    ...
    on tile[0]: test_rx(i_cfg[0], c_rx_hp, c_tx_hp, i_ctrl);
    ...
    on tile[0]: {
      ...
      test_rx(i_cfg[0], c_rx_hp, c_tx_hp, i_ctrl);
      #else
      test_rx(i_cfg[0], i_rx_lp[0], i_tx_lp[0], i_ctrl);
      #endif
    }
    ...
    on tile[0]: test_rx(i_cfg[0], i_rx_lp[0], i_tx_lp[0], i_ctrl);
