...
via the SMI serial interface. The application is tested on the ``XK-ETH-316-DUAL``
...
``lib_board_support`` for the PHY configuration on the ``XK-ETH-316-DUAL`` board.
...
(``main.xc``). In this examples the ports are set up for the PHY 0 on the ``XK-ETH-316-DUAL``
...
Actual port names such as ``XS1_PORT_1A`` are specified in the XN file ``xk-eth-316-dual-100m.xn`` and the source code refers
...
The demo uses the ``XK-ETH-316-DUAL`` board. By default, the board will
...
executed on the ``XK-ETH-316-DUAL`` board.
